...
class __Mustache_fe7d6c7148b295869978734b11db325e extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section7784de57cb2f5c98311592947c45f454($context, $indent, $value);
        $buffer .= ' ';
        $value = $context->find('user');
        if (empty($value)) {
...
';
    ...
    private function section7784de57cb2f5c98311592947c45f454(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
  <div class="flex h-10 w-1/3 justify-end" id="dashboard_loggedout">
    <a href="/login" class="flex justify-end">
      <button class="h-10 rounded-md bg-blue-500 px-4 py-2 text-white">
        Login
      </button>
    </a>
    <a href="/register" class="ml-4 flex justify-end">
      <button class="h-10 rounded-md bg-blue-500 px-4 py-2 text-white">
        Register
      </button>
    </a>
    <form action="/logout" method="POST" class="ml-4 flex justify-end">
      <button
        type="submit"
