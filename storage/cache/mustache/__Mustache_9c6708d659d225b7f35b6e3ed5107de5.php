...
class __Mustache_9c6708d659d225b7f35b6e3ed5107de5 extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->sectionC0897c92bfa56525c6650a6372f25462($context, $indent, $value);
        $value = $context->find('hasMorePages');
        $buffer .= $this->section2bd546b92084a00f31f46084d127f819($context, $indent, $value);
        $buffer .= $indent . '        </div>
';
        $buffer .= $indent . '      </div>
';
...
';
    ...
    private function sectionC0897c92bfa56525c6650a6372f25462(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
          <a href="?page={{currentPage - 1}}" class="btn btn-primary" {{^hasMorePages}}style="display:none"{{/hasMorePages}}>Previous</a>
          ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
...
';
    ...
    private function section2bd546b92084a00f31f46084d127f819(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        ...
        if (!is_string($value) && is_callable($value)) {
          ...
          ';
            ...
            foreach ($values as $value) {
                ...
                $value = $this->resolveValue($context->find('currentPage + 1'), $context);
                $buffer .= ($value === null ? '' : htmlspecialchars($value, 3, 'UTF-8'));
                $buffer .= '" class="btn btn-primary">Next</a>
';
                $context->pop();
            }
        }
