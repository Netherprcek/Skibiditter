...
class __Mustache_9f9ff2d5945eaf212cdf033c149ce69c extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->sectionD704745f5317a8b128d4202cc1d7494c($context, $indent, $value);
...
';
    ...
    private function sectionD704745f5317a8b128d4202cc1d7494c(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
          <a href="?page={{currentPage - 1}}" class="btn btn-primary bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2" {{^hasMorePages}}style="display:none"{{/hasMorePages}}>Previous</a>
          ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
            ...
            foreach ($values as $value) {
                ...
                $buffer .= ($value === null ? '' : htmlspecialchars($value, 3, 'UTF-8'));
                $buffer .= '" class="btn btn-primary bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2" ';
                $value = $context->find('hasMorePages');
                if (empty($value)) {
