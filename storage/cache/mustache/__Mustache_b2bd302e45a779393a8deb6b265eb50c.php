...
class __Mustache_b2bd302e45a779393a8deb6b265eb50c extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section03f53f5ab5b11478363de4908051036d($context, $indent, $value);
        $value = $context->find('hasMorePages');
        $buffer .= $this->section7577a6af16d5466910436d28cd6f19bb($context, $indent, $value);
        $buffer .= $indent . '        </div>
';
        $buffer .= $indent . '      </div>
';
...
';
    ...
    private function sectionBec8c415eaa24b8b347056a14efef5ed(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
          <a href="?page={{currentPage }}" class="btn btn-primary bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2">Previous</a>
          ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
...
';
    ...
    private function section03f53f5ab5b11478363de4908051036d(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        ...
        if (!is_string($value) && is_callable($value)) {
          ...
          ';
            ...
            foreach ($values as $value) {
                ...
                $buffer .= $this->sectionBec8c415eaa24b8b347056a14efef5ed($context, $indent, $value);
                $context->pop();
            }
        }
    ...
    private function section7577a6af16d5466910436d28cd6f19bb(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
          <a href="?page={{currentPage + 1}}" class="btn btn-primary bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Next</a>
          ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
            ...
            foreach ($values as $value) {
                ...
                $buffer .= ($value === null ? '' : htmlspecialchars($value, 3, 'UTF-8'));
                $buffer .= '" class="btn btn-primary bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Next</a>
';
                $context->pop();
            }
        }
