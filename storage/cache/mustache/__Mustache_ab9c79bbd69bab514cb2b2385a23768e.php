...
class __Mustache_ab9c79bbd69bab514cb2b2385a23768e extends Mustache_Template
{
...
';
        ...
        if (empty($value)) {
            ...
            $buffer .= $this->sectionB4510721d8df66b0a2aec22de8c92df0($context, $indent, $value);
            $buffer .= ' ';
        }
        $buffer .= '
';
...
';
    ...
    private function sectionB4510721d8df66b0a2aec22de8c92df0(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = ' <div class="mt-16"></div>{{> partials/followButton}} </div> ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
            } else {
