...
class __Mustache_e034796d6e32964a2da866cd297b171f extends Mustache_Template
{
...
';
        $buffer .= $indent . '      <h2>';
        $value = $this->resolveValue($context->findDot('post_text</h2>
    </div>
  </div>
  <div
    id="right_column"
    class="flex w-1/4 justify-start border-l-2 border-gray-800 pl-4"
  >
    <h2>{{session_user.username'), $context);
        $buffer .= ($value === null ? '' : htmlspecialchars($value, 3, 'UTF-8'));
        $buffer .= '</h2>
';
        $buffer .= $indent . '  </div>
